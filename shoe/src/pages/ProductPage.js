...
//     return Math.round(Math.abs((today - creationDate) / oneDay)) < 15;
//   };
...
//           // Deterministic Rating Formula
//           const firstSize = prod.variants[0]?.size;
//           let rating = 4.0;
//           if (firstSize === 1) rating = 4.2;
//           else if (firstSize === 2) rating = 4.1;

//           const reviews = Math.floor(Math.random() * (500 - 100) + 100);
//           setRatingInfo({ rating, reviews });
//         }
//       } catch (err) {
...
//   const isVideo = (url) =>
//     [".mp4", ".mov", ".webm", ".ogg"].some((ext) => url.endsWith(ext));
...
//       else await axios.post(`http://localhost:5000/api/wishlist/${product._id}`);
//       await fetchWishlist();
...
//   return (
//     <div className="container mx-auto px-4 py-8 md:py-16 bg-gradient-to-b from-zinc-50 to-zinc-100 font-sans">
...
//         >
//           <div
//             className="relative w-full h-[480px] bg-white rounded-2xl shadow-xl overflow-hidden group"
...
//                   isZoomed && window.innerWidth < 768 ? "scale-150" : ""
//                 }`}
//               />
//             )}
...
//                 className="absolute bottom-4 right-4 bg-white/80 backdrop-blur-sm p-3 rounded-full shadow-lg hover:bg-white transition"
//               >
//                 {isZoomed ? (
...
//               <div
//                 key={index}
//                 className={`flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border-2 cursor-pointer transition ${
//                   activeImageIndex === index
//                     ? "border-blue-800 scale-105 shadow-md"
...
//         {/* Details */}
//         <motion.div
//           className="w-full md:w-1/2 p-8 bg-white rounded-2xl shadow-2xl flex flex-col"
//           initial={{ opacity: 0, x: 40 }}
//           animate={{ opacity: 1, x: 0 }}
//           transition={{ duration: 0.7 }}
//         >
...
//               <span className="bg-blue-50 text-blue-700 text-xs font-semibold px-3 py-1 rounded-full tracking-wide">
//                 New Arrival
//               </span>
//             )}
...
//               >
//                 <FaShareAlt className="text-2xl text-gray-400 hover:text-blue-800" />
//               </button>
//             </div>
//           </div>
...
//                   key={i}
//                   className={`w-5 h-5 ${
//                     i < Math.floor(ratingInfo.rating)
//                       ? "text-yellow-400"
//                       : "text-gray-300"
//                   }`}
//                 />
...
//           <div className="mt-6">
//             <h4 className="font-semibold text-zinc-800 mb-2">Select Size</h4>
//             <div className="flex flex-wrap gap-3">
//               {product.variants.map((variant) => (
//                 <button
//                   key={variant.size}
//                   onClick={() => setSelectedVariant(variant)}
//                   className={`px-5 py-3 rounded-lg font-medium border transition-all ${
...
//                 className="w-full bg-gradient-to-r from-blue-900 to-blue-700 text-white font-bold py-4 rounded-lg text-lg shadow-lg"
//               >
//                 Add to Cart
//               </motion.button>
//             ) : (
...
//   const [isZoomed, setIsZoomed] = useState(false); // desktop zoom
//   const [isFullScreen, setIsFullScreen] = useState(false); // mobile zoom
...
//         await axios.delete(`http://localhost:5000/api/wishlist/${product._id}`);
//       } else {
//         await axios.post(`http://localhost:5000/api/wishlist/${product._id}`);
//       }
//       await fetchWishlist();
//       setHeartAnimation(true);
//       setTimeout(() => setHeartAnimation(false), 1000);
...
//                 className="md:hidden absolute bottom-4 right-4 bg-white/80 backdrop-blur-sm p-3 rounded-full shadow-lg hover:bg-white transition z-30"
